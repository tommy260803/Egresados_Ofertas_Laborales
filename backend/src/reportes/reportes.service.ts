...
export class ReportesService {
  ...
  async generarReporteSync(
    ...
    try {
      ...
      const { template, data } = await this.prepararReporte(
        tipo,
        filtrosValidados,
      );
      ...
      const { buffer, size } = await this.pdfService.generatePdfFromHtml(
        html,
        outputPath,
      );
  ...
  private formatFecha(fechaStr: string | undefined): string {
    if (!fechaStr) return "-";
  ...
  private resumenFiltros(filtros: ReporteFiltros) {
    return {
      fechaDesde: this.formatFecha(filtros.fechaDesde),
      fechaHasta: this.formatFecha(filtros.fechaHasta),
  ...
  private async prepararReporte(
    ...
    switch (tipo) {
      case "egresados_por_carrera": {
        ...
        return {
          template: "reporte_egresados_por_carrera",
          data: { ...base, titulo: "Listado de egresados por carrera", rows },
        };
      }
      case "ofertas_activas": {
        const rows = await this.reporteOfertasActivas(filtros);
        return {
          template: "reporte_ofertas_activas",
          data: { ...base, titulo: "Listado de ofertas activas", rows },
        };
      }
      case "postulaciones_por_oferta": {
        const rows = await this.reportePostulacionesPorOferta(filtros);
        return {
          template: "reporte_postulaciones_por_oferta",
          data: { ...base, titulo: "Postulaciones por oferta", rows },
        };
      }
      case "empleabilidad_por_carrera": {
        const rows = await this.reporteEmpleabilidadPorCarrera(filtros);
        return {
          template: "reporte_empleabilidad_por_carrera",
          data: { ...base, titulo: "Empleabilidad por carrera", rows },
        };
      }
      case "habilidades_mas_demandadas": {
        const rows = await this.reporteHabilidadesMasDemandadas(filtros);
        return {
          template: "reporte_habilidades_mas_demandadas",
          data: { ...base, titulo: "Top 10 habilidades mas demandadas", rows },
        };
      }
      default:
  ...
  private async reporteEgresadosPorCarrera(filtros: ReporteFiltros) {
    const qb = this.egresadoRepo
      ...
      .where("1=1");
    if (filtros.carrera)
      qb.andWhere("e.carrera ILIKE :carrera", {
  ...
  private async reporteOfertasActivas(filtros: ReporteFiltros) {
    const qb = this.ofertaRepo
      ...
      .addSelect(
        "CONCAT(COALESCE(o.salario_minimo::text, '-'), ' - ', COALESCE(o.salario_maximo::text, '-'))",
        "salario",
      )
      .addSelect("TO_CHAR(o.fecha_cierre, 'YYYY-MM-DD')", "fecha_cierre")
      .where("o.activa = true");
    if (filtros.sector)
      qb.andWhere("empresa.sector ILIKE :sector", {
  ...
  private async reportePostulacionesPorOferta(filtros: ReporteFiltros) {
    ...
    const qb = this.postulacionRepo
      ...
      .addSelect(
        "TO_CHAR(p.fecha_postulacion, 'YYYY-MM-DD')",
        "fecha_postulacion",
      )
      .where("o.id_oferta = :ofertaId", { ofertaId: filtros.ofertaId });
    if (fechaDesde)
      qb.andWhere("p.fecha_postulacion >= :fechaDesde", { fechaDesde });
    if (fechaHasta)
      qb.andWhere("p.fecha_postulacion <= :fechaHasta", { fechaHasta });
  ...
  private async reporteEmpleabilidadPorCarrera(filtros: ReporteFiltros) {
    ...
    const egresadosQb = this.egresadoRepo
      ...
      .groupBy("e.carrera");
    if (filtros.carrera)
      egresadosQb.where("e.carrera ILIKE :carrera", {
        carrera: `%${filtros.carrera}%`,
      });
    ...
    const contratadosQb = this.postulacionRepo
      ...
      .groupBy("e.carrera");
    if (filtros.carrera)
      contratadosQb.andWhere("e.carrera ILIKE :carrera", {
        carrera: `%${filtros.carrera}%`,
      });
    if (filtros.sector)
      contratadosQb.andWhere("empresa.sector ILIKE :sector", {
        sector: `%${filtros.sector}%`,
      });
    if (fechaDesde)
      contratadosQb.andWhere("p.fecha_postulacion >= :fechaDesde", {
        fechaDesde,
      });
    if (fechaHasta)
      contratadosQb.andWhere("p.fecha_postulacion <= :fechaHasta", {
        fechaHasta,
      });
    ...
    const mapContratados = new Map(
      contratados.map((item) => [item.carrera, Number(item.contratados) || 0]),
    );
