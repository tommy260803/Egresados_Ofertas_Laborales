...
export class HabilidadService {
  ...
  async findAll(filters?: { search?: string; categoria?: string }): Promise<Habilidad[]> {
    const qb = this.habilidadRepo.createQueryBuilder('h');
    if (filters?.search) {
      qb.andWhere('(h.nombre_habilidad ILIKE :search OR h.categoria ILIKE :search)', { search: `%${filters.search}%` });
    }
    if (filters?.categoria) {
      qb.andWhere('h.categoria ILIKE :categoria', { categoria: `%${filters.categoria}%` });
    }
    qb.orderBy('h.categoria', 'ASC').addOrderBy('h.nombre_habilidad', 'ASC');
    return qb.getMany();
  }

  async create(dto: { nombre_habilidad: string; categoria?: string }): Promise<Habilidad> {
  ...
  async update(id: number, dto: { nombre_habilidad?: string; categoria?: string }): Promise<Habilidad> {
    const habilidad = await this.habilidadRepo.findOneBy({ id_habilidad: id });
    if (!habilidad) throw new NotFoundException('Habilidad no encontrada');
