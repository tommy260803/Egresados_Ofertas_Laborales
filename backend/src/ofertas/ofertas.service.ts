...
export class OfertasService {
  ...
  async create(empresaId: number, dto: any): Promise<OfertaLaboral> {
    ...
    const oferta = this.ofertaRepo.create({ ...ofertaData, empresa } as any) as unknown as OfertaLaboral;
  ...
  async findAll(filters?: Record<string, unknown>): Promise<OfertaLaboral[]> {
    const qb = this.ofertaRepo.createQueryBuilder('o')
      .leftJoinAndSelect('o.empresa', 'e')
      .leftJoinAndSelect('o.habilidades', 'h')
      .where('o.activa = true');
    if (filters?.titulo) qb.andWhere('o.titulo ILIKE :tit', { tit: `%${filters.titulo as string}%` });
    if (filters?.modalidad && filters.modalidad !== 'todas') {
      // Normalizar modalidad para la consulta: quitar tildes y minúsculas
      const mod = (filters.modalidad as string).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      qb.andWhere('o.modalidad = :mod', { mod });
    }
    if (filters?.ubicacion) qb.andWhere('o.ubicacion ILIKE :ubi', { ubi: `%${filters.ubicacion as string}%` });
    if (filters?.salario_min) qb.andWhere('o.salario_maximo >= :min', { min: filters.salario_min as number });
    if (filters?.salario_max) qb.andWhere('o.salario_minimo <= :max', { max: filters.salario_max as number });
    if (filters?.habilidad) {
      qb.innerJoin('o.habilidades', 'hf').andWhere('hf.id_habilidad = :hid', { hid: filters.habilidad as number });
    }
    return qb.getMany();
  }

  async findOne(id: number): Promise<OfertaLaboral> {
    const oferta = await this.ofertaRepo.findOne({ where: { id_oferta: id }, relations: ['empresa', 'habilidades'] });
    if (!oferta) throw new NotFoundException('Oferta no encontrada');
    return oferta;
  }

  async update(id: number, empresaId: number, dto: UpdateOfertaDto): Promise<OfertaLaboral> {
    const oferta = await this.findOne(id);
    if (oferta.empresa.id_empresa !== empresaId) throw new ForbiddenException('No es tu oferta');
  ...
  async postular(egresadoId: number, dto: PostularDto): Promise<Postulacion> {
    ...
    const existente = await this.postulacionRepo.findOne({
      where: { oferta: { id_oferta: dto.id_oferta }, egresado: { id_egresado: egresadoId } }
    });
    if (existente) throw new ForbiddenException('Ya postulaste');
    const egresado = await this.egresadoRepo.findOneBy({ id_egresado: egresadoId });
    if (!egresado) throw new NotFoundException('Egresado no encontrado');
    const postulacion = this.postulacionRepo.create({
      oferta, egresado, mensaje: dto.mensaje, cv_postulacion_url: dto.cv_url,
    } as any) as unknown as Postulacion;
    return this.postulacionRepo.save(postulacion);
  }

  async cambiarEstadoPostulacion(postulacionId: number, empresaId: number, dto: UpdatePostulacionEstadoDto): Promise<Postulacion> {
    ...
    if (postulacion.oferta.empresa.id_empresa !== empresaId) throw new ForbiddenException();
    postulacion.estado_actual = dto.estado;
    // El trigger en BD guardará historial automáticamente
    return this.postulacionRepo.save(postulacion);
  }

  async listarPostulacionesPorEgresado(egresadoId: number): Promise<Postulacion[]> {
  ...
  async listarPostulacionesPorEmpresa(empresaId: number): Promise<Postulacion[]> {
    return this.postulacionRepo.find({
  ...
  async listarPostulacionesPorOferta(ofertaId: number, empresaId: number): Promise<Postulacion[]> {
    const oferta = await this.findOne(ofertaId);
    if (oferta.empresa.id_empresa !== empresaId) throw new ForbiddenException();
    return this.postulacionRepo.find({ where: { oferta: { id_oferta: ofertaId } }, relations: ['egresado'] });
  }

  async listarOfertasPorEmpresa(empresaId: number, filters?: Record<string, any>): Promise<any[]> {
    const qb = this.ofertaRepo.createQueryBuilder('o')
    ...
    if (filters?.activa !== undefined && filters.activa !== null) {
      qb.andWhere('o.activa = :act', { act: filters.activa === 'true' || filters.activa === true });
    }
  ...
  async alternarEstadoOferta(id: number, empresaId: number): Promise<OfertaLaboral> {
    const oferta = await this.findOne(id);
    if (oferta.empresa.id_empresa !== empresaId) throw new ForbiddenException();
  ...
  async obtenerHistorialPostulacion(postulacionId: number, egresadoId: number): Promise<HistorialEstado[]> {
    const postulacion = await this.postulacionRepo.findOne({
      where: { id_postulacion: postulacionId, egresado: { id_egresado: egresadoId } }
    });
  ...
  async obtenerHistorialPostulacionEmpresa(postulacionId: number, empresaId: number): Promise<HistorialEstado[]> {
    const postulacion = await this.postulacionRepo.findOne({
      where: { id_postulacion: postulacionId, oferta: { empresa: { id_empresa: empresaId } } }
    });
    if (!postulacion) throw new NotFoundException('Postulación no encontrada');
