...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    return { user: { id: usuario.id_usuario, email: usuario.email, rol: usuario.rol }, ...tokens };
  }
  ...
  async login(dto: LoginDto) {
    ...
    if (!user || !(await bcrypt.compare(dto.contrasena, user.contrasena_hash))) {
      throw new UnauthorizedException('Credenciales inválidas');
    }
    const tokens = this.generateTokens(user);
    return { user: { id: user.id_usuario, email: user.email, rol: user.rol }, ...tokens };
  }

  async refreshToken(refreshToken: string) {
    try {
      const payload = this.jwtService.verify(refreshToken, { secret: this.config.get('JWT_REFRESH_SECRET') });
      const user = await this.usuarioRepo.findOneBy({ id_usuario: payload.sub });
